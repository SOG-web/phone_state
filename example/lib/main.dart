...
import 'package:phone_state_example/state.dart';
import 'dart:ui';
...
void onStart(ServiceInstance service) async {
  ...
  Timer.periodic(const Duration(seconds: 1), (timer) async {
    ...
    // stateValue.number.addListener(() {
    //   service.invoke(
    //     'update',
    //     {
    //       "current_date": DateTime.now().toIso8601String(),
    //       "number": stateValue.number.value,
    //     },
    //   );
    // });
      ...
      setState(() {
        if (event != null) {
          ...
          if (event.number != null) {
            stateValue.number.value = event.number!;
          }
        }
      });
      // log(event.toString());
